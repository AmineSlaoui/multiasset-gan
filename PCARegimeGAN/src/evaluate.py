...
def full_evaluation(
    ...
    # Inter-temporal: average over multiple paths for robust estimation
    T_real = real.shape[0]
    if fake.ndim == 3:
        n_it_paths = min(fake.shape[0], 20)
        acf_scores_all, vc_scores_all, lev_scores_all = [], [], []
        for p in range(n_it_paths):
            it_p = intertemporal_scores(real, fake[p])
            acf_scores_all.append(it_p["acf_score"])
            vc_scores_all.append(it_p["vc_score"])
            lev_scores_all.append(it_p["lev_score"])
        results["acf_score"] = float(np.mean(acf_scores_all))
        results["vc_score"] = float(np.mean(vc_scores_all))
        results["lev_score"] = float(np.mean(lev_scores_all))
        results["acf_std"] = float(np.std(acf_scores_all))
        results["vc_std"] = float(np.std(vc_scores_all))
        results["lev_std"] = float(np.std(lev_scores_all))
    else:
        it = intertemporal_scores(real, fake_flat[:T_real])
        results.update(it)
