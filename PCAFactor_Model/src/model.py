...
class FiLMLayer(nn.Module):
    ...
    def forward(self, h, cov):
        ...
        CAUSAL: Each timestep t is modulated only by cov at time t.
        No future macro information leaks into earlier timesteps.
        """
        # Per-timestep conditioning (causal — no future leak)
        # cov: (B, d_cov, T) → permute to (B, T, d_cov) → MLP → (B, T, 2*hidden)
        B, _, T = cov.shape
        cov_t = cov.permute(0, 2, 1)  # (B, T, d_cov)
        params = self.mlp(cov_t)  # (B, T, 2*hidden_dim)
        gamma, beta = params.chunk(2, dim=2)  # Each (B, T, hidden_dim)
        gamma = gamma.permute(0, 2, 1)  # (B, hidden_dim, T)
...
class Generator(nn.Module):
    ...
    def __init__(self, n_assets=60, n_factors=5, d_z=10, d_cov=15,
                 ...
                 film_position="last"):
        """
        film_position: "last" (after final block), "every" (after each block),
                       "middle" (after block 3)
        """
        super().__init__()
        ...
        # TCN backbone: individual blocks (not Sequential, for FiLM insertion)
        self.input_proj = nn.Conv1d(d_z, hidden_dim, 1)
        self.blocks = nn.ModuleList([
            ResidualBlock(hidden_dim, 2, 2 ** i, dropout) for i in range(num_blocks)
        ])

        # FiLM layers based on position
        if film_position == "every":
            self.films = nn.ModuleList([FiLMLayer(d_cov, hidden_dim) for _ in range(num_blocks)])
        elif film_position == "middle":
            self.films = nn.ModuleList([None] * num_blocks)
            self.films[num_blocks // 2] = FiLMLayer(d_cov, hidden_dim)
        else:  # "last"
    ...
    def forward(self, z_full, cov_full, alpha_hat, beta_hat, sigma_hat, factors):
        ...
        # 1. TCN on noise only, with FiLM at configured position
        cov_input = cov_full[:, :, :-1]
        h = self.input_proj(z_full[:, :, :-1])
        for i, block in enumerate(self.blocks):
            h = block(h)
            if self.film_position == "every":
                h = self.films[i](h, cov_input)
            elif self.film_position == "middle" and self.films[i] is not None:
                h = self.films[i](h, cov_input)
        if self.film_position == "last":
