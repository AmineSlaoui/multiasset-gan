...
class ProperValDataset:
    """Validation dataset where windows must overlap the validation period,
    with a per-window mask so downstream metrics can drop training-period
    days that land inside the window's output span.

    ``sample_batch`` returns the same keys as ``SequenceDataset`` plus
    ``val_mask`` of shape ``(B, t_l)`` — True where the day is in
    ``[val_start_idx, T)`` and False where it falls in the pre-val training
    span. Flattening by the mask before computing correlation-matrix
    statistics removes the training-period contamination in the earliest
    windows (see ``train_v9`` validation loop).
    """
    def __init__(self, returns, factors, covariates,
    ...
    def sample_batch(self, batch_size, device):
        ...
        mask_list = []
        for idx in indices:
            e = idx + self._inner.t_full
            cov_list.append(self._inner.covariates[idx:e])
            ts = idx + self.rfs
            te = ts + self.t_l
            ret_list.append(self._inner.returns[ts:te])
            factor_list.append(self._inner.factors[ts:te])
            alpha_list.append(self._inner.alpha_hat[ts:te])
            beta_list.append(self._inner.beta_hat[ts:te])
            sigma_list.append(self._inner.sigma_hat[ts:te])
            mask_list.append(torch.arange(ts, te) >= self.val_start_idx)
        return {
            "covariates": torch.stack(cov_list).permute(0, 2, 1).to(device),
            "returns": torch.stack(ret_list).permute(0, 2, 1).to(device),
            "factors": torch.stack(factor_list).permute(0, 2, 1).to(device),
            ...
            "sigma_hat": torch.stack(sigma_list).permute(0, 2, 1).to(device),
            "beta_hat": torch.stack(beta_list).to(device).permute(0, 2, 3, 1),
            "val_mask": torch.stack(mask_list).to(device),
        }
