...
def run_one_r2(r2, real_factors_oos, sigma_f, beta_hats, real_test, model_paths_per_rebal,
    ...
    ``model_paths_per_rebal[m][k]`` is the ``(n_paths, rebal_freq, N)`` segment
    produced by the walk-forward generator at rebalance k (see
    ``walk_forward.gen_paths_walk_forward`` / ``fb_paths_walk_forward``).
    """
    N = real_test.shape[1]
    ...
    for k, rp in enumerate(rebal_points):
        ...
        for name, per_rebal in model_paths_per_rebal.items():
            paths = per_rebal[k]
            flat = paths.reshape(-1, N) if paths.ndim == 3 else paths
...
def main():
    ...
    # ── Walk-forward path generation (strictly-past conditioning per rebal) ─
    print("  FB (walk-forward) ...")
    ...
    print("  FB+shrunk (walk-forward) ...")
    np.random.seed(args.seed)
    fb_s_segments = fb_paths_walk_forward(
        fb_ctor=lambda: FactorBootstrap(window=126),
        returns=returns.values, factors=f_arr,
        rebal_points_abs=rebal_abs, rebal_freq=args.rebal_freq,
    ...
    print("  fixed-η (v7d, walk-forward) ...")
    ...
    fixed_segments = gen_paths_walk_forward(
        g7, cov_arr, f_arr, alpha_hat, beta_hat, sigma_hat,
        rebal_points_abs=rebal_abs, rebal_freq=args.rebal_freq,
        n_paths=args.n_paths, device=device,
    )

    print("  SF-MarketGAN-R (walk-forward) ...")
    g9, cfg9 = load_sfmg(args.sfmg_ckpt, N, n_factors, d_cov, None, device)
    ...
    paths_by_rebal = {
        "FB":         fb_segments,
        "FB+shrunk":  fb_s_segments,
        "Fixed-η":    fixed_segments,
        "SF-MG-R":    v9_segments,
    }
