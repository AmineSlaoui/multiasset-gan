...
bootstrap by resampling asset indices with replacement.

This reduces the wall clock from O(hours) to O(seconds) and gives
identical results for the mean-of-per-asset metrics (ACF, VC, LEV).
...
def main():
    ...
    fixed_paths = gen_paths_v9_like(g7, cov_arr, f_arr, alpha_hat, beta_hat, sigma_hat,
    ...
    v9_paths = gen_paths_v9_like(g9, cov_arr, f_in, alpha_hat, beta_hat, sigma_hat,
                                  test_start_idx, n_paths=args.n_paths,
                                  t_l=real_test.shape[0], device=device)
    ...
    for name, paths in method_paths.items():
        # Flatten paths to (P*T, N) for per-asset series. Use path 0 only for
        # ACF-type stats (standard per-path estimation; averaging across paths
        # is done in paper eval but here we take path 0 as representative to
        # keep bootstrap cheap). Alternatively: take per-path ACF, average.
        P, T, _ = paths.shape
        p0 = paths[0]
        acf_fake = per_asset_acf(p0, args.max_lag)
        vc_fake  = per_asset_acf(p0 ** 2, args.max_lag)
        lev_fake = per_asset_leverage(p0, args.max_lag)
        ...
        print(f"    {name}: per-asset ACF mean={per_asset_dist[name]['ACF'].mean():.3f}  "
              f"VC mean={per_asset_dist[name]['VC'].mean():.3f}")
