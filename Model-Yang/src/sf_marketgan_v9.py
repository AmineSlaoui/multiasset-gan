...
class GeneratorV9(nn.Module):
    ...
    def __init__(self, n_assets=60, n_factors=11, d_z=10, d_cov=15,
        ...
        # Fixed v7d-trained shrunk correlation Cholesky (unit-diagonal)
        if residual_cholesky is not None:
            self.register_buffer("L_rho", torch.tensor(residual_cholesky, dtype=torch.float32))
        else:
            self.L_rho = None
